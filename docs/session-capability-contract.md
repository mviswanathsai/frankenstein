...
`set_usage` merges the provided fields into the current usage object. Fields present in the input overwrite the current values; absent fields are left untouched. How a service maintains usage between explicit `set_usage` calls — auto-computing from record writes, leaving it to the kernel, or a mix — is an implementation decision. The contract requires only that `set_usage` is available and behaves as described.
