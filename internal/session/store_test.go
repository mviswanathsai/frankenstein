...
func TestCreateAndResumeSession(t *testing.T) {
	...
	if created.Records[0].Text == nil || *created.Records[0].Text != "start here" {
		t.Fatalf("initial record text = %s, want start here", stringPtrOrNil(created.Records[0].Text))
	}
...
func TestReadAndMaterializeOrderedRecord(t *testing.T) {
	...
	if read.Records[1].Role != "assistant" || stringPtrOrNil(read.Records[1].Text) != "next" {
		t.Fatalf("Read() second record = (%q,%s), want assistant next", read.Records[1].Role, stringPtrOrNil(read.Records[1].Text))
	}
...
func TestReadOrdersRecordsBySessionSeq(t *testing.T) {
	...
	if stringPtrOrNil(read.Records[1].Text) != "second" || stringPtrOrNil(read.Records[2].Text) != "third" {
		t.Fatalf("records returned in wrong order: got %s then %s", stringPtrOrNil(read.Records[1].Text), stringPtrOrNil(read.Records[2].Text))
	}
}
...
func TestIdempotentMutationDoesNotApplyTwice(t *testing.T) {
	...
	if stringPtrOrNil(second.Records[1].Text) != "once" {
		t.Fatalf("second record text = %s, want once", stringPtrOrNil(second.Records[1].Text))
	}
}
